...
const char start_main[] = "sem"; // names of semaphore
...
int main()
{
    ...
    // open semaphore
    if ((main_sem = sem_open(start_main, O_CREAT, 0666, 1)) == 0)
    {
    ...
    // open shared memory
    if ((shm_descriptor = shm_open(shm_name, O_CREAT | O_EXCL | O_RDWR, 0666)) == -1)
    {
