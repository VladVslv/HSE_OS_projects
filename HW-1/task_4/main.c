...
int main(int argc, char **argv)
{
    ...
    int pipe_st[2]; // pipe between second and third process
    ...
    if (pipe(pipe_st) < 0)
    {
        perror("First process: can\'t open pipe between second and third processes\n");
        exit(-1);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            if (close(pipe_st[1]) < 0)
            {
                perror("Third process: can\'t close reading side of pipe\n");
                exit(-1);
            }

            // write to buffer from the pipe
            str_size = read(pipe_st[0], buffer, BUFFER_SIZE);
            if (str_size < 0)
            {
                perror("Third process: can\'t read from the pipe\n");
                exit(-1);
            }
            if (close(pipe_st[0]) < 0)
            {
