...
const int BUFFER_SIZE = 5;                      // size of buffer
...
int main(int argc, char **argv)
{
  ...
  do
  {
    ...
    if (str_size < 0)
    {
      perror("First process: can\'t read from the input file\n");
      unlink(pipe_fs_name);
      unlink(pipe_sf_name);
      exit(-1);
    }
    // write to the pipe from buffer
    if (write(pipe_fs, buffer, str_size) < 0)
    {
      perror("First process: can\'t write to the pipe\n");
      unlink(pipe_fs_name);
      unlink(pipe_sf_name);
      exit(-1);
    }
  ...
  do
  {
    ...
    if (write(output, buffer, str_size) != str_size)
    {
